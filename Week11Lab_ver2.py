...
if __name__ == '__main__':
    ...
    output_file = sys.argv[3] if len(sys.argv) > 3 else "counts.csv"
    ...
    with CapeMCA() as mca:
        try:
            ...
            while time.time() - start < duration:
                ...
                timestamp = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(read_start))
    ...
    if interval_rows:
        ...
        print(f"Saved interval data to {output_file}")
